...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //총 지출
    public double totalExpenditureByAccountBook(Long accountBookId){
        AccountBook accountBook = accountBookRepository.findById(accountBookId).orElseThrow(()-> new NoSuchElementException("AccountBook not found"));
    ...
    //일자별 통계
    public Map<String, Object> getDailyStatistics(Long accountBookId, String date){
        //가계부 찾기
        AccountBook accountBook = accountBookRepository.findById(accountBookId).orElseThrow(()-> new NoSuchElementException("AccountBook not found"));
        ...
        //해당 가계부, 해당 지출 날짜의 지출 목록
        List<Expense> expenses = expenseRepository.findExpensesByAccountBookIdAndDate(accountBookId, LocalDate.parse(date));

        Map<String, Double> expenditureByCategory = calculateExpenseByCategory(expenses, currencyToAvgExchangeRate); //해당 날짜의 카테고리별 지출
        Map<String, Double> expenditureByPaymentMethod = calculateExpenseByPaymentMethod(expenses, currencyToAvgExchangeRate); //해당 날짜의 결제방법별 지출

        Map<String, Object> dailyStatistics = new HashMap<>();
        dailyStatistics.put("expenditureByCategory", expenditureByCategory);
        dailyStatistics.put("expenditureByPaymentMethod", expenditureByPaymentMethod);

        return dailyStatistics;
    }

    //카테고리별 지출
    public Map<String, Double> getExpenditureByCategory(Long accountBookId){
        //가계부 찾기
        AccountBook accountBook = accountBookRepository.findById(accountBookId).orElseThrow(()-> new NoSuchElementException("AccountBook not found"));
